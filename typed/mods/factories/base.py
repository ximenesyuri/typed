...
def ListFactory(*args):
    ...
    if args and all(isinstance(t, (TYPE, ABSTRACT)) for t in args):
        ...
        sorted_types = tuple(sorted(unique, key=_key))
        if sorted_types != args:
            return ListFactory(*sorted_types)
    if not args:
        return type(None)
    if len(args) == 1 and (callable(args[0]) or hasattr(args[0], 'func')) and not isinstance(args[0], (TYPE, ABSTRACT)):
        f = args[0]
        if isinstance(f, Typed):
            domain_type = ListFactory(*f.domain)
            codomain_type = ListFactory(f.codomain)
...
def SetFactory(*args):
    ...
    if args and all(isinstance(t, (TYPE, ABSTRACT)) for t in args):
        ...
        sorted_types = tuple(sorted(unique, key=_key))
        if sorted_types != args:
            return SetFactory(*sorted_types)
    ...
    if len(args) == 1 and (callable(args[0]) or hasattr(args[0], 'func')) and not isinstance(args[0], (TYPE, ABSTRACT)):
        f = args[0]
        if isinstance(f, Typed):
            domain_type = SetFactory(*f.domain)
            codomain_type = SetFactory(f.codomain)
...
@cache
def DictFactory(*args, keys=None):
    from typed.mods.types.base import TYPE, ABSTRACT
    from typed.mods.types.func import Typed
    T = (Typed, TYPE, ABSTRACT)
    if args and all(isinstance(t, (TYPE, ABSTRACT)) for t in args):
        ...
        sorted_types = tuple(sorted(unique, key=_key))
        if sorted_types != args:
            return DictFactory(*sorted_types, keys=keys)
    ...
    if len(args) == 1 and (callable(args[0]) or hasattr(args[0], 'func')) and not isinstance(args[0], (TYPE, ABSTRACT)):
        f = args[0]
        if isinstance(f, Typed):
            domain_type = DictFactory(f.domain, keys=keys)
            codomain_type = DictFactory(f.codomain, keys=keys)
